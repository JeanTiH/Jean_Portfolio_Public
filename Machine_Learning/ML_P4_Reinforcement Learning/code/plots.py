...
class Plots:
    @staticmethod
    def grid_world_policy_plot(data, label, map_size, savepath):
        if not math.modf(math.sqrt(len(data)))[0] == 0.0:
            warnings.warn("Grid map expected.  Check data length")
        else:
            data = np.around(np.array(data).reshape((map_size, map_size)), 2)
            ...
            colorbar.set_ticklabels(['Left', 'Down', 'Right', 'Up'], fontsize=16)
            plt.title(label)
    ...
    @staticmethod
    def grid_values_heat_map(data, label, map_size):
        if not math.modf(math.sqrt(len(data)))[0] == 0.0:
            warnings.warn("Grid map expected.  Check data length")
        else:
            data = np.around(np.array(data).reshape((map_size, map_size)), 2)
...
if __name__ == "__main__":
    ...
    Plots.grid_world_policy_plot(np.array(new_pi), "Grid World Policy", 4)
    ...
    # V, V_track, pi = Planner(frozen_lake.env.P).value_iteration()
    V, V_track, pi = Planner(frozen_lake.env.P).policy_iteration()
    Plots.grid_values_heat_map(V, "State Values", 4)