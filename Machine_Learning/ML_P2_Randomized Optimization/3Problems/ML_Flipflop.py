...
----------------------------------------------------------------------
'''
def Flipflop(random_seed, size, state):
    # Store the stats
    if state:
        path = '/Users/jjhan/PycharmProjects/pythonProject/CN/ML1/0Project2/3Problems/stats/Flipflop'
    else:
        path = None
    ...
    # 2. Parameters after tuning
    if size == 30:
        restart_list = [0]
        decay_list = [mlrose.ArithDecay]
        temperature_list = [0.1]

        population_sizesGA = [25]
        mutation_rates = [0.25]

        population_sizesMC = [40]
        ...
        population_sizesGA = [100]
        mutation_rates = [0.3]

        population_sizesMC = [250]
        keep_percent_list = [0.35]

    # 4.1 Randomized Hill Climbing (RHC)
    start_time = time.time()
    rhc = RHCRunner(problem=problem, restart_list=restart_list,
                    experiment_name='Flipflop-RHC' + str(size) + '_seed' + str(random_seed), output_directory=path,
    ...
    sa = SARunner(problem=problem, decay_list=decay_list, temperature_list=temperature_list,
                  experiment_name='Flipflop-SA' + str(size) + '_seed' + str(random_seed),
    ...
    ga = GARunner(problem=problem, population_sizes=population_sizesGA, mutation_rates=mutation_rates,
                  experiment_name='Flipflop-GA' + str(size) + '_seed' + str(random_seed),
                  output_directory=path,
                  iteration_list=[2000], seed=random_seed)
    ...
    mc = MIMICRunner(problem=problem, population_sizes=population_sizesMC, keep_percent_list=keep_percent_list,
                     experiment_name='Flipflop-MIMIC' + str(size) + '_seed' + str(random_seed),
                     output_directory=path,
...
def postprocess(size, random_seeds, problem, state):
    ...
    for random_seed in random_seeds:
        ...
        time_RHC, time_SA, time_GA, time_MC = Flipflop(random_seed, size, state)
